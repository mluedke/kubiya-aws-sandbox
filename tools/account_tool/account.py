...
set_account_budget = AWSCliTool(
    ...
    content="aws sts assume-role --role-arn arn:aws:iam::{account_id}:role/OrganizationAccountAccessRole --role-session-name session-{account_id} && aws budgets create-budget --account-id {account_id} --budget '{json_data}'".format(
